...
def get_aqi_details(aqi):
    if aqi <= 50:
        ...
        return "Moderate", "#eab308", "Air quality is acceptable."
    elif aqi <= 150:
        return (
        ...
        return "Very Unhealthy", "#a855f7", "Health warnings of emergency conditions."
    else:
        return "Hazardous", "#7f1d1d", "Health warning of emergency conditions."
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "bar": {
                    "color": "rgba(0,0,0,0)"
                },  # Hide bar, use needle logic if needed or just threshold
...
def main():
    ...
    today_range = (
        f"Low: {today_summary.get('min_aqi', 0):.0f} - High: {today_summary.get('max_aqi', 0):.0f}"
        if today_summary
        else "N/A"
    )
    ...
    with col_meta:
        ...
        if train_rmse_val and test_rmse_val:
            ...
            if gap < 1.5:
                ...
                explanation = (
                    "⚠️ <strong>Moderate Gap:</strong> Some difference, but acceptable."
                )
                color_status = "#eab308"
            else:
                explanation = (
                    "❌ <strong>Overfitting Risk:</strong> Large gap detected."
                )
                color_status = "#ef4444"
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="#334155", title="AQI Level"),
